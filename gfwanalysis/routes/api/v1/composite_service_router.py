...
from gfwanalysis.middleware import get_geo_by_hash, get_instrument, get_geo_date_range, get_thumb_size

composite_service_v1 = Blueprint('composite_service_v1', __name__)

def composite(geojson, instrument, date_range, thumb_size):
    """
    Get a composite satellite image for a geostore id.
    """
    try:
        data = CompositeService.get_composite_image(geojson=geojson, instrument=instrument, date_range=date_range, thumb_size=thumb_size)
...
@get_geo_date_range
@get_thumb_size
def trigger_compositing(geojson, area_ha, instrument, date_range, thumb_size):
    """Get composite image for given geostore"""
    return composite(geojson, instrument, date_range, thumb_size)
