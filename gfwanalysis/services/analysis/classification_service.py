...
class ClassificationService(object):
    ...
    def classify(img_id):
        ...
        try:
            ...
            #logging.info(f'[classification_service]: attempting to get image {image.getInfo()}')
            logging.info(f"[classification_service]: model = {model}")
            # Apply classifer to specified L8 or S2 image
            classified_image = image.classify(model)
            logging.info(f"[classification_service]: classified version = {classified_image}")
            
            # Generate tile url and return in d['url'] object
            url = get_url(classified_image) #(classified_image)
...
def get_image(img_id):
    ...
    try:
        result_img = ee.Image(img_id).divide(100 * 100)
...
def get_model_from_local():
    try:
        # find a way of checking if model is in local memory (probably using OS?)
        #discard following 3 lines, needs to be imported from docker environment  
        # use OS to test if model is saved as a local file
        model = pickle.load(open("opt/downl_classif.pkl", 'rb'))       #hardcoded for now
        logging.info(f'[classification_service]: model from memory: {model}')
        return model
    except:
        return None
